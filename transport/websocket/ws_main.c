...
static aprs_ws_server_t* g_srv = NULL;

static void on_signal(int sig) {
    (void)sig;
    printf("\nShutting down...\n");
    if (g_srv) aprs_ws_server_stop(g_srv);
}
...
int main(int argc, char** argv) {
    ...
    g_srv = aprs_ws_server_create(port, engine);
    if (!g_srv) {
        fprintf(stderr, "Failed to create WebSocket server on port %d\n", port);
        aprs_engine_destroy(engine);
        return 1;
    }

    printf("WebSocket server listening on ws://localhost:%d\n", port);
    printf("Protocol: JSON over WebSocket (v1.0)\n");
    printf("Connect with:  wscat -c ws://localhost:%d\n", port);
    printf("\nPress Ctrl+C to stop\n\n");

    int result = aprs_ws_server_run(g_srv);

    aprs_ws_server_destroy(g_srv);
    aprs_engine_destroy(engine);
    return result;
}
